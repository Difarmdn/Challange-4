...
const getAllCar = () => {
  fetch("/getcars")
    ...
    .then((hasil) => {
      const body = document.getElementById("dataCars");
...
const filterCar = (char, date, time, passenger) => {
  ...
  let newCar = AllCar.filter((car) => {
    return car.available.toString().includes(char) && new Date(car.availableAt) < tanggal && car.capacity.toString() >= passenger;
  });
  const body = document.getElementById("dataCars");
